...
def CheckWordIsValid(Word, AllowedWords):
  left, right = 0, len(AllowedWords) - 1
  while left <= right:
    middle = (left+right) // 2
    if AllowedWords[middle] == Word:
      return middle
    elif AllowedWords[middle] < Word:
      left = middle + 1
    else:
      right = middle - 1
  return -1
...
def Main():
  ...
  MaxTilesPlayed = 50
  NoOfEndOfTurnTiles = 3
  StartHandSize = int(input("Enter a start hand size: "))
  ...
  while Choice != "9": 
    ...
    if Choice == "1":
      ...
      PlayGame(AllowedWords, TileDictionary, False, StartHandSize, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles)
