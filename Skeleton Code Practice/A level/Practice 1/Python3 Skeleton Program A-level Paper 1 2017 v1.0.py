...
class Simulation:
  def __init__(self, LandscapeSize, InitialWarrenCount, InitialFoxCount, Variability, FixedInitialLocations, AvgProbabilityOfDeathOtherCauses):
    self.__ViewRabbits = ""
    self.__TimePeriod = 0
    ...
    self.__AvgProbabilityOfDeathOtherCauses = AvgProbabilityOfDeathOtherCauses
    self.__Landscape = []
    ...
    while (self.__WarrenCount > 0 or self.__FoxCount > 0) and MenuOption != 5:
      ...
      if MenuOption == 1:
        ...
        self.__AdvanceTimePeriod(self.__AvgProbabilityOfDeathOtherCauses)
      if MenuOption == 2:
        self.__TimePeriod += 1
        self.__ShowDetail = False
        self.__AdvanceTimePeriod(self.__AvgProbabilityOfDeathOtherCauses)
  ...
  def __InputCoordinate(self, CoordinateName):
    ...
    while Coordinate >= self.__LandscapeSize or Coordinate < 0:
      Coordinate = int(input("Coordinate is outside of landscape, please try again.\n"))
    return Coordinate
  
  def __AdvanceTimePeriod(self,AvgProbabilityOfDeathOtherCauses):
    NewFoxCount = 0
    ...
    for x in range (0, self.__LandscapeSize):
      for y in range (0, self.__LandscapeSize):
        if not self.__Landscape[x][y].Warren is None:
          ...
          self.__Landscape[x][y].Warren.AdvanceGeneration(self.__ShowDetail, AvgProbabilityOfDeathOtherCauses)
          if self.__ShowDetail:
  ...
  def __CreateLandscapeAndAnimals(self, InitialWarrenCount, InitialFoxCount, FixedInitialLocations):
    for x in range (0, self.__LandscapeSize):
      for y in range (0, self.__LandscapeSize):
        if x == 5 or y == 2:
          self._Landscape[x][y] = Location("R")
        else:
  ...
  def __CreateNewWarren(self):
    ...
    while not self.__Landscape[x][y].Warren is None or self._Landscape[x][y].Terrain = "R":
      x = random.randint(0, self.__LandscapeSize - 1)
      y = random.randint(0, self.__LandscapeSize - 1)
    if self.__ShowDetail:
  ...
  def __CreateNewFox(self):
    x = random.randint(0, self.__LandscapeSize - 1)
    y = random.randint(0, self.__LandscapeSize - 1)
    while not self.__Landscape[x][y].Fox is None or self._Landscape[x][y].Terrain = "R":
      x = random.randint(0, self.__LandscapeSize - 1)
      y = random.randint(0, self.__LandscapeSize - 1)
    if self.__ShowDetail:
  ...
  def CheckIfPathCrossesRiver(self, FoxX, FoxY, WarrenX, WarrenY):
    if Foxx - Warrenx > 0:
      xChange = 1
    else:
      xChange = -1
    if WarrenX != FoxX:
      x = WarrenX + xChange
      while x != FoxX:
        if self._Landscape[x][FoxY].Terrain == "R":
          return True
        x += xChange
      if FoxY - WarrenY > 0:
        yChange = 1
    ...
    for FoxX in range(0, self.__LandscapeSize):
      for FoxY in range (0, self.__LandscapeSize):
        if not self.__Landscape[FoxX][FoxY].Fox is None:
          if not self.CheckIfPathCrossesRiver(FoxX, FoxY, WarrenX, WarrenY):
    ...
    for y in range (0, self.__LandscapeSize):
      ...
      for x in range (0, self.__LandscapeSize):
        ...
        if not self.__Landscape[x][y].Fox is None:
          ...
          print(" ", end = "")
        print(self._Landscape[x][y].Terrain, end="")
        print("|", end = "")
      print()
...
class Warren:
  ...
  def AdvanceGeneration(self, ShowDetail, AvgProbabilityOfDeathOtherCauses):
    self.__PeriodsRun += 1
    if self.__RabbitCount > 0:
      self.__KillByOtherFactors(ShowDetail)
    if self.__RabbitCount > 0:
      self.__AgeRabbits(ShowDetail, AvgProbabilityOfDeathOtherCauses)
  ...
  def __AgeRabbits(self, ShowDetail, AvgProbabilityOfDeathOtherCauses):
    DeathCount = 0
    for r in range (0, self.__RabbitCount):
      self.__Rabbits[r].CalculateNewAge(AvgProbabilityOfDeathOtherCauses)
...
class Rabbit(Animal):
  ...
  def CalculateNewAge(self, AvgProbabilityOfDeathOtherCauses):
    self._Age += 1
    if self.__Gender == Genders.Male:
      self._ProbabilityOfDeathOtherCauses = AvgProbabilityOfDeathOtherCauses * 1.5
    if self.__Gender == Genders.Female:
      if self._Age <= 2:
        self._ProbabilityOfDeathOtherCauses = 0.05
      else:
        self._ProbabilityOfDeathOtherCauses = AvgProbabilityOfDeathOtherCauses + 0.05
    if self._Age >= self._NaturalLifespan:
      self._IsAlive = False
  ...
  while MenuOption != 3:
    ...
    if MenuOption == 1 or MenuOption == 2:
      if MenuOption == 1:
        ...
        AvgProbabilityOfDeathOtherCauses = 0
      else:
        ...
        AvgProbabilityOfDeathOtherCauses = 0
      Sim = Simulation(LandscapeSize, InitialWarrenCount, InitialFoxCount, Variability, FixedInitialLocations, AvgProbabilityOfDeathOtherCauses)
  input()
