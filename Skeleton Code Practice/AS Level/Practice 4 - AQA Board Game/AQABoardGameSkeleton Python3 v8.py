...
def DisplayErrorCode(ErrorNumber):
    if ErrorNumber == 1:
        print("Error 1: The selected piece cannot be moved. "
              "It is not in the list of possible moves.")
    elif ErrorNumber == 2:
        print("Error 2: The selected destination is invalid. "
              "That piece cannot move to the chosen row and column.")
    elif ErrorNumber == 3:
        print("Error 3: Invalid input. "
              "Row and column must be entered as whole numbers.")
    elif ErrorNumber == 4:
        print("Error 4: The game file could not be opened or found. "
              "The saved game could not be loaded.")
    else:
        print("Error", ErrorNumber, ": An unknown error has occurred.")
...
def PrintResult(A, B, NextPlayer):
  ...
  CountNumberOfPieces(PlayersPieces)
...
def CountNumberOfPieces(PlayersPieces):
  Count = 0
  for i in range(1, NUMBER_OF_PIECES + 1):
      if PlayersPieces[i][ROW] > -1:
          Count += 1
      return Count

def PrintResults(A, B, NextPlayer):
    print('Game Ended')
    ...
    if TotalA < TotalB:
        print(f'A won this game with a score of {TotalA}\nB got a score of {TotalB}')
    elif TotalB < TotalA:
        print(f'B won this game with a score of {TotalB}\nA got a score of {TotalA}')
    else:
        print(f'IT WAS A DRAW! Both PLAYERS GVOT A SCRE OF {TotalA}!')
    PrintPlayerPieces(A, B)
