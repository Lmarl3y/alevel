...
def SendSignals(MorseCodeString):
  Transmission = EMPTYSTRING
  CodeStringLength = len(MorseCodeString)
  for i in range(CodeStringLength):
    ...
    if Symbol == '.':
      ...
      SymbolString = SPACE + SPACE
    Transmission = Transmission + SymbolString
    print(Transmission)

def OutputAlphabetWithCode(Letter, MorseCode):
    for i in range(1, 27):
        print(Letter[i], end=SPACE)
        print('{0:<5}'.format(MorseCode[i]), end=SPACE)
        if i % 4 == 0:
            print()
    print()
...
def DisplayMenu():
  ...
  print("A - Output Alphabet with MorseCode")
...
def SendReceiveMessages():
  ...
  for i in range(3):
      ...
      while not ValidDisplacement:
          try:
              Displacement = int(input('Enter encryption key: '))
