  ...
  for Row in range(FIELDLENGTH):
    for Column in range(FIELDWIDTH):
      if Field[Row][Column] == PLANT:
        NumberOfPlants += 1
        totalcells = FIELDLENGTH * FIELDWIDTH
        percentage = NumberOfPlants / totalcells * 100
  if NumberOfPlants == 1:
...
def SaveToFile(Field):
    decision = input('Save the current Field state to a text file? (Y/N):').upper()
    if decision == "Y":
        filename = input('Enter what you would like to call the file: ')
        with open(f'{filename}.txt', 'w') as f:
            for Row in range(FIELDLENGTH):
                line = "".join(Field[Row])
                f.write(line + f"| {str(Column)}" + "\n")
            f.close()
            print(f'done in {filename}.txt')
    else:
        return
